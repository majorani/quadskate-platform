...
export default function ComoFuncionaPage() {
  ...
  const STEPS = [
    ...
    { num: '04', label: t('step4Label'), title: t('step4Title'), desc: t('step4Desc'),
    ...
    { num: '05', label: t('step5Label'), title: t('step5Title'), desc: t('step5Desc'),
      icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg> },
    { num: '06', label: t('step6Label'), title: t('step6Title'), desc: t('step6Desc'),
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: '#f4f1e8', fontFamily: "'Inter', system-ui, sans-serif" }}>
      <style>{`
        ...
        .step-card {
          ...
          border-right: 1px solid #1f1f1f; padding: 28px 18px;
          cursor: default; transition: flex 0.45s cubic-bezier(0.25,0.46,0.45,0.94), background 0.3s ease;
        ...
        .step-card:hover .step-expanded { opacity: 1; max-height: 300px; pointer-events: auto; }
        .feature-row {
          display: flex; align-items: center; gap: 12px;
          font-size: 12px; color: #555; font-family: 'Manrope', sans-serif; margin-bottom: 10px;
        }
        .feature-row::before { content: ''; width: 18px; height: 1px; background: ${GOLD}; flex-shrink: 0; }
        @media (max-width: 768px) {
          .steps-row { flex-direction: column !important; }
          .step-card {
            flex: none !important; border-right: none !important;
            border-bottom: 1px solid #1f1f1f; padding: 24px 20px !important; min-height: unset !important;
          }
          .step-card:last-child { border-bottom: none; }
          .step-card:hover { flex: none !important; }
          .step-expanded { opacity: 1 !important; max-height: none !important; pointer-events: auto !important; }
      ...
      <div style={{ maxWidth: MAX_W, margin: '0 auto', padding: `clamp(56px, 8vw, 100px) ${pad}` }}>
        <div ref={setElRef(0)} style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 36 }}>
          ...
          <span className="mono" style={{ fontSize: 10, fontWeight: 500, letterSpacing: '0.28em', color: GOLD, textTransform: 'uppercase' }}>{t('eyebrow')}</span>
        </div>
        <div ref={setElRef(1)}>
          <h1 className="arch" style={{ fontSize: 'clamp(44px, 7vw, 88px)', lineHeight: 0.95, letterSpacing: '-0.02em', textTransform: 'uppercase', marginBottom: 28, color: '#f4f1e8' }}>
            {t('heroTitle1')}<br />{t('heroTitle2')}<span style={{ color: GOLD }}> {t('heroTitleGold')}</span>
          </h1>
        </div>
        <div ref={setElRef(2)}>
          <p className="manrope" style={{ fontSize: 'clamp(14px, 1.8vw, 17px)', color: '#8a8a82', maxWidth: 480, lineHeight: 1.7, fontWeight: 300 }}>
            {t('heroDesc')}<br /><strong style={{ color: '#f4f1e8', fontWeight: 700 }}>{t('heroDescBold')}</strong> {t('heroDescEnd')}
          </p>
        </div>
      </div>
        ...
        <div style={{ maxWidth: MAX_W, margin: '0 auto', padding: `clamp(28px, 4vw, 48px) ${pad} 0` }}>
          <div ref={setElRef(3)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12, marginBottom: 32 }}>
            <h2 className="arch" style={{ fontSize: 'clamp(22px, 3vw, 38px)', textTransform: 'uppercase', letterSpacing: '-0.02em', color: '#f4f1e8', lineHeight: 1 }}>
              {t('stepsTitle')} <span style={{ color: GOLD }}>{t('stepsTitleGold')}</span> {t('stepsTitleEnd')}
            </h2>
            <span className="mono" style={{ fontSize: 10, color: '#444', letterSpacing: '0.3em', textTransform: 'uppercase' }}>{t('stepsCount')}</span>
          </div>
        </div>

        <div style={{ maxWidth: MAX_W, margin: '0 auto', borderTop: '1px solid #1f1f1f' }}>
          <div ref={setElRef(4)} className="steps-row" onMouseLeave={() => setHovered(null)} style={{ display: 'flex' }}>
            {STEPS.map((step, i) => {
              const isHov = hovered === i
              return (
                <div key={i} className="step-card" onMouseEnter={() => setHovered(i)} style={{ background: isHov ? '#111' : '#0a0a0a' }}>
                  <div>
                    <div className="arch" style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.06em', color: isHov ? '#f4f1e8' : '#555', transition: 'color 0.3s ease', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {step.label}
                    </div>
                  </div>
                  <div className="step-expanded">
                    <div style={{ marginTop: 24 }}>
                      <div style={{ width: 38, height: 38, borderRadius: '50%', border: `1px solid ${GOLD}55`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: GOLD, marginBottom: 16 }}>
                        {step.icon}
                      </div>
                      <h3 className="arch" style={{ fontSize: 'clamp(15px, 1.6vw, 18px)', textTransform: 'uppercase', letterSpacing: '-0.01em', color: '#f4f1e8', marginBottom: 10, lineHeight: 1.1 }}>
                        {step.title}
                      </h3>
                  ...
                  <div className="arch" style={{ fontSize: 72, lineHeight: 1, color: 'transparent', WebkitTextStroke: `1px ${isHov ? GOLD + '55' : '#1e1e1e'}`, transition: 'all 0.3s ease', userSelect: 'none', pointerEvents: 'none', letterSpacing: '-0.04em', alignSelf: 'flex-end' }}>
                    {step.num}
                  </div>
                </div>
              )
        ...
        <div style={{ maxWidth: MAX_W, margin: '0 auto', padding: `clamp(40px, 6vw, 72px) ${pad}` }}>
          <div ref={setElRef(5)} style={{ textAlign: 'center', marginBottom: 'clamp(36px, 5vw, 56px)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, justifyContent: 'center', marginBottom: 14 }}>
              ...
              <span className="mono" style={{ fontSize: 9, color: GOLD, letterSpacing: '0.3em', textTransform: 'uppercase' }}>{t('publishedEyebrow')}</span>
              <div style={{ width: 32, height: 1, background: '#1f1f1f' }} />
            </div>
          ...
          <div ref={setElRef(6)} className="published-strip" style={{ display: 'flex', border: '1px solid #1f1f1f' }}>
            <div className="published-half-left" style={{ flex: 1, padding: 'clamp(28px, 4vw, 48px)', borderRight: '1px solid #1f1f1f' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
                ...
                <span className="mono" style={{ fontSize: 9, color: GOLD, letterSpacing: '0.3em', textTransform: 'uppercase', fontWeight: 500 }}>{t('skaterEyebrow')}</span>
              </div>
              ...
              {([t('skaterF1'), t('skaterF2'), t('skaterF3')] as string[]).map((f, i) => (
            ...
            <div style={{ flex: 1, padding: 'clamp(28px, 4vw, 48px)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: GOLD, display: 'inline-block' }} />
                <span className="mono" style={{ fontSize: 9, color: GOLD, letterSpacing: '0.3em', textTransform: 'uppercase', fontWeight: 500 }}>{t('judgeEyebrow')}</span>
              </div>
              <h3 className="arch" style={{ fontSize: 'clamp(18px, 2.2vw, 26px)', textTransform: 'uppercase', letterSpacing: '-0.01em', color: '#f4f1e8', marginBottom: 14, lineHeight: 1 }}>
                {t('judgeTitle')}
              </h3>
              ...
              {([t('judgeF1'), t('judgeF2'), t('judgeF3')] as string[]).map((f, i) => (
                <div key={i} className="feature-row">{f}</div>
              ))}
            </div>
          </div>
        ...
        <div style={{ maxWidth: MAX_W, margin: '0 auto', padding: `clamp(56px, 8vw, 100px) ${pad}`, textAlign: 'center' }}>
          <div ref={setElRef(7)}>
            <h2 className="arch" style={{ fontSize: 'clamp(28px, 5vw, 64px)', textTransform: 'uppercase', letterSpacing: '-0.02em', color: '#f4f1e8', lineHeight: 1, marginBottom: 18 }}>
              {t('ctaTitle')} <span style={{ color: GOLD }}>{t('ctaTitleGold')}</span>
            </h2>
            ...
            <button
              ...
              style={{ background: btnHov ? GOLD_L : GOLD, border: 'none', padding: '16px 44px', color: '#000', fontWeight: 900, fontSize: 12, cursor: 'pointer', letterSpacing: '0.2em', textTransform: 'uppercase', fontFamily: "'Archivo Black', sans-serif", display: 'inline-flex', alignItems: 'center', gap: 10, transition: 'background 0.2s ease' }}
            >
              {t('ctaButton')}
