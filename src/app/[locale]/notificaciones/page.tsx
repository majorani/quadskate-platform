...
export default function NotificacionesPage() {
  const t = useTranslations('NotificacionesPage')
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: '#e8e8e8' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px' }}>
        ...
        {loading && (
          <div style={{ color: '#444', fontSize: 11, letterSpacing: 2, textTransform: 'uppercase' }}>{t('loading')}</div>
        )}

        {!loading && notifs.length === 0 && (
          <div style={{ borderTop: '1px solid #2a2a2a', padding: '60px 0', textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: '#333', letterSpacing: 3, textTransform: 'uppercase' }}>{t('empty')}</div>
          </div>
        )}
