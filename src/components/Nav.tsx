...
function MobileLink({ label, onClick, active, gold }: { label: string, onClick: () => void, active: boolean, gold?: boolean }) {
  return (
    <button onClick={onClick} style={{ background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', padding: '18px 0', fontSize: 22, fontWeight: 900, letterSpacing: -0.5, textTransform: 'uppercase', color: active ? '#C9A84C' : gold ? '#C9A84C' : '#e8e8e8', borderBottom: '1px solid #1a1a1a', width: '100%' }}>
      {label}
    </button>
  )
}