...
export default function EventCard({ event: ev }: { event: Event }) {
  ...
  return (
    ...
    >
      {/* Flyer o placeholder */}
      {ev.flyer_url ? (
        <div style={{ height: 180, background: `url(${ev.flyer_url}) center/cover no-repeat`, borderBottom: '1px solid #2a2a2a' }} />
      ) : (
        <div style={{ height: 180, background: '#111', borderBottom: '1px solid #2a2a2a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
