...
export default function EventCard({ event: ev }: { event: Event }) {
  ...
  return (
    <div
      ...
      style={{ background: '#0a0a0a', cursor: 'pointer', position: 'relative', overflow: 'hidden', transition: 'background .15s' }}
    ...
    >
      {ev.flyer_url ? (
        ...
        <div style={{ height: 180, background: '#111', borderBottom: '1px solid #2a2a2a', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 4, color: '#2a2a2a', textTransform: 'uppercase' }}>{t('noFlyer')}</span>
          {badge}
        </div>
      )}
